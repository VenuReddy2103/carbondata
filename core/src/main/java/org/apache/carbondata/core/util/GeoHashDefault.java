...
public class GeoHashDefault implements CustomIndex<Long, String, List<Long[]>> {
  ...
  public void init(String handlerName, Map<String, String> properties) throws Exception {
    ...
    if (originLongitude == null || originLatitude == null) {
      throw new MalformedCarbonCommandException(
              String.format("%s property is invalid. Must specify %s and %s properties.",
                      CarbonCommonConstants.INDEX_HANDLER, ORIGIN_LONGITUDE, ORIGIN_LATITUDE));
    }
    ...
    // 计算过程先把maxLongitude, maxLatitude 代入计算出N，如果N不是整数，则取N的下一个整数，
    // 代入后求得Xmax，Ymax。
    this.calculateArea();
    // 计算区域被划分成多少行多少列
  ...
  private void gridExpansionArea() {
    ...
    this.udfRowStartNumber =
            (int) ((this.CalculateMaxLongitude - this.userDefineMaxLongitude) / this.deltaY);
    this.udfRowEndNumber = this.totalRowNumber;
    this.udfCloumnEndNumber =
            (int) ((this.CalculateMaxLatitude - this.userDefineMaxLatitude) / this.deltaX);
