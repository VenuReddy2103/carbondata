...
public class PolygonExpression extends UnknownExpression implements ConditionalExpression {
  private static final Logger LOGGER =
      LogServiceFactory.getLogService(PolygonExpression.class.getName());
  private String polygon;
  private CustomIndex<List<Long[]>> handler;
  private List<Long> inList = new ArrayList<Long>();
  ...
  private void processExpression() {
    try {
      ...
      validate(ranges);
      LOGGER.info("Using collections binarySearch. Ranges size: " + ranges.size() + " inList size: "
          + inList.size());
    } catch (Exception e) {
    ...
    while (low <= high) {
      ...
      if (searchForNumber >= range[0]) {
        if (searchForNumber <= range[1]) {
          ...
          // Number is bigger than this range's min and max. Search on the right side of this range.
          low = mid + 1;
        }
      } else {
        // Number is smaller than this range's min and max. Search on the left side of this range.
        high = mid - 1;
      }
    }
    return false;
  ...
  public ExpressionResult evaluate(RowIntf value) {
    if (Collections.binarySearch(inList, (Long) value.getVal(0)) < 0) {
      return falseExpRes;
    }
    return trueExpRes;
  }
